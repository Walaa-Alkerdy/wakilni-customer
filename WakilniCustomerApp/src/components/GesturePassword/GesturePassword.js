...
export default class GesturePassword extends Component {
    ...
    onLayout = (e) => {
        this.setState({
            mainWidth: e.nativeEvent.layout.width,
    ...
    _setToActive(point) {
        ...
        if (this.state.points[0].isActive && this.state.points[2].isActive) {
            // must add point 1
            if (this.convertArrayToString(this._sequence).includes('02') || this.convertArrayToString(this._sequence).includes('20')) {
                this._sequence.pop()
                this._addSequence(this.state.points[1].index)
                this.state.points[1].isActive = true
            }
        }

        if (this.state.points[0].isActive && this.state.points[8].isActive) {
            // must add point 4
            if (this.convertArrayToString(this._sequence).includes('08') || this.convertArrayToString(this._sequence).includes('80')) {
                this._sequence.pop()
                this._addSequence(this.state.points[4].index)
                this.state.points[4].isActive = true
            }
        }

        if (this.state.points[0].isActive && this.state.points[6].isActive) {
            // must add point 3
            if (this.convertArrayToString(this._sequence).includes('06') || this.convertArrayToString(this._sequence).includes('60')) {
                this._sequence.pop()
                this._addSequence(this.state.points[3].index)
                this.state.points[3].isActive = true
            }
        }

        if (this.state.points[2].isActive && this.state.points[8].isActive) {
            // must add point 5
            if (this.convertArrayToString(this._sequence).includes('28') || this.convertArrayToString(this._sequence).includes('82')) {
                this._sequence.pop()
                this._addSequence(this.state.points[5].index)
                this.state.points[5].isActive = true
            }
        }

        if (this.state.points[2].isActive && this.state.points[6].isActive) {
            // must add point 4
            if (this.convertArrayToString(this._sequence).includes('26') || this.convertArrayToString(this._sequence).includes('62')) {
                this._sequence.pop()
                this._addSequence(this.state.points[4].index)
                this.state.points[4].isActive = true
            }
        ...
        if (this.state.points[6].isActive && this.state.points[8].isActive) {
            // must add point 7
            if (this.convertArrayToString(this._sequence).includes('68') || this.convertArrayToString(this._sequence).includes('86')) {
                this._sequence.pop()
                this._addSequence(this.state.points[7].index)
                this.state.points[7].isActive = true
            }
        }

        //add final point reached to the sequence
        this._addSequence(point.index)
    ...
    _onTouchStart = (e, gestureState) => {
        ...
        this._reset()
        let location = {
            x: e.nativeEvent.pageX - this.state.mainX,
            y: e.nativeEvent.pageY - (this.state.mainY + 70 + (Platform.OS === "ios" ? 70 : (70 + StatusBar.currentHeight))),
        }
